...
function Get-ConnectionStatus {
    param($LastConnected, $LastDisconnected)
    # If we have both timestamps, compare them
    if ($LastConnected -and $LastDisconnected) {
        if ($LastDisconnected -gt $LastConnected) {
            return "DISCONNECTED"
        } else {
            return "CONNECTED"
        }
    }
    # Only a disconnect time exists — was unplugged, never re-plugged in this session
    if ($LastDisconnected -and -not $LastConnected) {
        return "DISCONNECTED"
    }
    # Only a connect time exists — plugged in, never logged an unplug
    if ($LastConnected -and -not $LastDisconnected) {
        return "CONNECTED"
    }
    return "UNKNOWN"
}
...
function Write-DeviceRow {
    ...
    $status = Get-ConnectionStatus -LastConnected $LastConnected -LastDisconnected $LastDisconnected
    ...
    # ── Header row: index + name + status badge ──────────────────
    ...
    # ── Disconnected notice (shown only when device is unplugged) ─
    if ($status -eq "DISCONNECTED") {
        $sinceMsg = ""
        if ($LastDisconnected) {
            $span = (Get-Date) - $LastDisconnected
            if ($span.TotalDays -ge 1) {
                $sinceMsg = "  ($([math]::Floor($span.TotalDays))d ago)"
            } elseif ($span.TotalHours -ge 1) {
                $sinceMsg = "  ($([math]::Floor($span.TotalHours))h ago)"
            } else {
                $sinceMsg = "  ($([math]::Floor($span.TotalMinutes))m ago)"
            }
        }
        Write-Host "       " -NoNewline
        Write-Host "⚠  Device is currently unplugged — last seen $ld$sinceMsg" -ForegroundColor DarkYellow
    }
    ...
    Write-Host "  ▲ In: " -NoNewline -ForegroundColor DarkGreen
    Write-Host $lc -NoNewline -ForegroundColor Green
    Write-Host "  ▼ Out: " -NoNewline -ForegroundColor DarkRed
    Write-Host $ld -ForegroundColor Red

    # ── Detail row: VID/PID + serial ─────────────────────────────
    $details = @()
...
foreach ($d in $allDevices) {
    $s = Get-ConnectionStatus -LastConnected $d.LastConnected -LastDisconnected $d.LastDisconnected
...
Write-Host "⊕ First Plugged In  " -NoNewline -ForegroundColor Yellow
Write-Host "▲ Last Plugged In  " -NoNewline -ForegroundColor Green
