...
# Method 1: timestamp comparison  (best, when registry has data)
# Method 2: WMI Win32_PnPEntity   (live query — works without timestamps)
# Method 3: ConfigFlags reg bit   (0x04 = device not currently present)
...
function Write-DeviceRow {
    ...
    if ($status -eq "DISCONNECTED") {
        ...
        Write-Host "       " -NoNewline
        if ($ld -eq "─────────────────") {
            Write-Host "⚠  Device is currently unplugged (registry has no unplug timestamp)" -ForegroundColor DarkYellow
        } else {
...
# Pre-load WMI device list once — used as the fallback presence check.
# Win32_PnPEntity.DeviceID matches the InstanceId format in the registry.
$script:WmiPnpIds = @()
try {
    $script:WmiPnpIds = @(
...
foreach ($d in $allDevices) {
    $s = Get-ConnectionStatus `
        -LastConnected $d.LastConnected -LastDisconnected $d.LastDisconnected `
...
if (@($stor).Count -eq 0) {
    ...
    foreach ($d in $stor) {
        Write-DeviceRow -FriendlyName $d.FriendlyName -DeviceType $d.DeviceType `
            ...
            -FirstInstall $d.FirstInstall -LastConnected $d.LastConnected `
            -LastDisconnected $d.LastDisconnected -Index $i
        $i++
    }
}
